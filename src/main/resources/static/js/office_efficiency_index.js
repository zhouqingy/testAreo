...
function loadChannelBaseInfo(data){
	...
	$("#base-info .device-score").find(".score-val").text(15);
	...
	$("#base-info .weigui").find(".score-val").text(0);
	//设置门店基本信息-营销评级
	...
	$("#base-info .yingxiao").find(".score-val").text(3);
	//设置门店基本信息-受理速度
	var avgTimeScore=0;
	...
	$("#base-info .avgtime").find(".score-val").text('15min');
	//设置门店基本信息-业务量
	var yewuliangVal=0;
	...
	$("#base-info .yewuliang").find(".score-val").text(1);
	//设置门店基本信息-排队机耗时
	var channelHandleInfo=data.channelHandleInfo;
	...
	$("#base-info").find(".channel-total-score").text('0');
	...
	$("#base-info").find(".chanenl-star").append('0');
    ...
    $("#base-info").find(".popval").text("总试飞数" || "总试飞数");
    var rePortData={};
    //厅办理业务平均耗时
...
function loadChannelHandleDetail(data){
	var option = {
		    ...
		    series: [{
		        name: 'RTR',type: 'line',smooth: true,lineStyle: {normal: {width: 2}},
		        yAxisIndex:0,
		        areaStyle: {
		            normal: {
		        ...
		        name: 'TTC',type: 'line',smooth: true,lineStyle: { normal: {width: 2}},
		        yAxisIndex:0,
		        areaStyle: {
		            normal: {
		                color: new echarts.graphic.LinearGradient(0, 0, 0, 1, [{
		        ...
		        name: 'ACROAMATICS',type: 'line',smooth: true,lineStyle: {normal: {width: 2}},
		        yAxisIndex:1,
		        areaStyle: {
		            normal: {
		                color: new echarts.graphic.LinearGradient(0, 0, 0, 1, [{
	...
	for(var key in staffHandleInfo){
		...
		var tr1=[
			...
			'<td colspan="3">',
					...
					'<div class="avg-time-label">&nbsp;&nbsp;</div>',
					'<div class="avg-time-value"></div>',
				'</td>',
			'</tr>',
			'<tr>',
				'<td> ',
					'<div class="staff-cust-time">',
					   '<span style="font-size:15px;">机长</span><br/><span style="color:#00A8FE;font-size:18px;font-weight:600;">***</span>',
					'</div>',
				'</td>',
				'<td>',
					'<div class="staff-order-count">',
					    '<span style="font-size:15px;">机高</span><br/><span style="color:#00A8FE;font-size:18px;font-weight:600;">***</span>',
					'</div>',
				'</td>',
				'<td>',
					'<div class="staff-alarm">',
					   '<span style="font-size:15px;">翼展</span><br/><span style="color:#00A8FE;font-size:18px;font-weight:600;">***</span>',
					'</div>',
			    '</td>',
			'</tr> ',
		];
	...
	for(var key in deviceInfo){
		...
		var tr1=[
				'<tr> ',                                                            
					...
					'</td> ',                                                           
				    '<td><div class="label-name">服务器1</div></td> ',                            
				    '<td rowspan="3" colspan="2" class="kuan-dai"> ',                   
				            '<div class="progress-label">上行宽度速率 ***</div>',       
							'<div style="width:180px;height:15px;"> ',                         
								...
								'</div>',                                                    
							'</div> ',                                                      
				            '<div class="progress-label">下行宽度速率 ***</div>',   
							'<div style="width:180px;height:15px;">',                      
								'<div class="progress" style="float:left;height:8px;width:180px;background-color:#383E60;">',
			...
			tr2=[
				...
				'<tr> ',                                                            
					'<td rowspan="3" class="device-img">',                            
					    '<img src="/images/'+gradeImg+'"/>',
					'</td> ',                                                           
					'<td><div class="label-name">服务器2</div></td> ',                              
				    '<td rowspan="3" colspan="2" class="kuan-dai"> ',                   
				            '<div class="progress-label">上行宽度速率 ***</div>',       
							'<div style="width:180px;height:15px;"> ',                         
								'<div class="progress" style="float:left;height:8px;width:180px;background-color:#383E60;">',
...
function loadPageData(){
	 ...
	 if(Object.keys(groupChannelInfo).length==0){
		 ...
		 if(undefined==groupChannelInfo || null==groupChannelInfo){
			 groupChannelInfo={"channelCode":"11228790","groupChannelCode":"3201001965930","channelName":"试飞实验室"};
	     }else{
